/* global nw:false */
import ILaunchable from './scripts/models/ILaunchable'

const os = require('os')
const net = require('net')
const gui = require('nw.gui')
const win = gui.Window.get()
import renderTemplate from './scripts/gui/renderTemplate'
import parseAll from './scripts/parsing/parseAll'
import setResults from './scripts/gui/setResults'
import escapeHtml from './scripts/utils/escapeHtml'
import inputFocusClassToBody from './scripts/utils/inputFocusClassToBody'
import disableKeyDownForElement from './scripts/utils/disableKeyDownForElement'
import getConfig from './scripts/getConfig'
import filterSearchItems from './scripts/filterSearchItems'
import openItem from './scripts/openItem'
...
function onAppChange(e) {
  ...
  if (val.length > 1) {
    const desktopItems = store.searchItems.filter((item: ILaunchable) => item.desktop)
    // if we can't find it in the desktop friendly list, then try harder
    matchingApp =
      desktopItems.find((item: ILaunchable) => item.name.startsWith(val)) ||
...
function launch() {
  ...
  ;($('app') as HTMLInputElement).value = store.withApp = $('ghost').innerHTML = ''
  store.ghost = null
  hide()
}
